...
export function helperSearch(
  ...
  if (!node.noLink && node.title.toLowerCase().includes(query.toLowerCase())) {
    res.push({ ...node, items: undefined, href: `/${section}${nextLink}` });
    parentHas = true;
  }
...
export function advanceSearch(query: string) {
  return Object.entries(ROUTES).flatMap(([section, routes]) => 
    routes.map((node) =>
      helperSearch(query, node, "", section, 1, query.length == 0 ? 2 : undefined)
    ).flat()
  );
}
