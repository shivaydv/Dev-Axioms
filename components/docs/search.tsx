      ...
      >
        ...
        <DialogContent className="p-0 max-w-[650px] sm:top-[38%] top-[45%] !rounded-md">
          ...
          {filteredResults.length == 0 && searchedInput && (
            <p className="text-muted-foreground mx-auto my-4 text-sm text-center">
              No results found for{" "}
              <span className="text-primary font-medium">{`"${searchedInput}"`}</span>
            </p>
          )}
          <ScrollArea className="max-h-[400px] overflow-y-auto">
            <div className="flex flex-col items-start overflow-y-auto sm:px-2 px-1 ">
              {filteredResults.map((item) => {
                ...
                return (
                  <DialogClose key={item.href} asChild>
                    <Anchor
                      className={cn(
                        "dark:hover:bg-stone-900/70 hover:bg-stone-100 w-full px-3 rounded-md text-sm flex items-center justify-between transition-colors",
                        paddingClass
                      )}
                      href={item.href}
                    >
                      ...
                      >
                        <FileIcon className="h-[1.1rem] w-[1.1rem] text-muted-foreground" />{" "}
                        {item.title}
                      </div>
                      <div className="text-xs text-muted-foreground px-2 truncate max-w-[180px]">
                        {item.href.split('/').slice(1,2)}
                      </div>
                    </Anchor>
                  </DialogClose>
                );
